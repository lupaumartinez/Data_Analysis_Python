...
def gather_data(path_from, R2th, totalbins, monitor):
    ...
    for i in range(L):
        ...
        #Invierto columnas y NPs
        # list_of_col[i] = int(NP.split('_')[3])
        # list_of_nps[i] = int(NP.split('_')[1])    
        ############################################ import beta
        ...
        # extended_list_of_beta_good_array.append(data)
        ...
        key = 'col_%02d_np_%02d' % (list_of_col[i], list_of_nps[i])
        list_of_beta_good_err_dict[key] = np.array(data)
    ...
    ##### Recuperar data de seccion eficaz de PL
    list_of_mean_PL_per_sec_per_irrad = np.zeros((L))
    list_of_TOTAL_PL_per_sec_per_irrad = np.zeros((L))
    for i in range(L):
        NP = list_of_folders[i] 
        list_of_col[i] = int(NP.split('_')[1])
        list_of_nps[i] = int(NP.split('_')[3])
        ...
        ############################################ import beta
        PL_folder = os.path.join(path_from, NP, 'pl_in_bins')
        PL_filename = 'PL_cts_per_sec_per_irrad_%s.dat' % NP
        PL_filepath = os.path.join(PL_folder, PL_filename)
        # print('CULO')
        # print(i)
        # print(beta_filepath)

        data = np.loadtxt(PL_filepath, skiprows=0)
        try:
            list_of_mean_PL_per_sec_per_irrad[i] = data[0]
            list_of_TOTAL_PL_per_sec_per_irrad[i] = data[2]

        except:
            
            list_of_mean_PL_per_sec_per_irrad[i] = np.nan
            list_of_TOTAL_PL_per_sec_per_irrad[i] = np.nan
        
    list_of_TOTAL_PL_per_sec_per_irrad_clean = [x for x in list_of_TOTAL_PL_per_sec_per_irrad if str(x) != 'nan']
    list_of_TOTAL_PL_per_sec_per_irrad_clean=np.asarray(list_of_TOTAL_PL_per_sec_per_irrad_clean)
    
    plt.figure()
    nbins = 10
    list_of_mean_PL_per_sec_per_irrad_N=list_of_mean_PL_per_sec_per_irrad/(1e6)
    #range_tuple = [0.8*min(list_of_mean_PL_per_sec_per_irrad_N),1.2*max(list_of_mean_PL_per_sec_per_irrad_N)]
    range_tuple = [0,15]
    plt.hist(list_of_mean_PL_per_sec_per_irrad_N, bins=nbins, range=range_tuple, rwidth = 1, \
...
#    ax.get_yaxis().set_ticks([])
    ax.set_ylim([0, 40])
    PL_median=round(np.median(list_of_mean_PL_per_sec_per_irrad_N),1)
    PL_std=round(np.std(list_of_mean_PL_per_sec_per_irrad_N),1)
    ax.axvline(np.median(list_of_mean_PL_per_sec_per_irrad_N), ymin = 0, ymax = 1, color='k', linestyle='--', linewidth=2,label="{0} ± {1}".format(PL_median, PL_std))
    plt.legend()
    ax.set_xlim(range_tuple)
    ax.set_xlabel(u'PL cross section (1e6 cts µm$^{2}$/mW seg EM)')
    ax.set_ylabel('Entries')
    aux_folder = manage_save_directory(path_from,'stats')
    figure_name = os.path.join(aux_folder,'hist_of_PL_Cross_Section.png' )
    ...
    plt.figure()
    nbins = 10
    list_of_TOTAL_PL_per_sec_per_irrad_N=list_of_TOTAL_PL_per_sec_per_irrad_clean/(1e5)
    #range_tuple = [0.8*min(list_of_mean_PL_per_sec_per_irrad_N),1.2*max(list_of_mean_PL_per_sec_per_irrad_N)]
    range_tuple = [0,15]
    plt.hist(list_of_TOTAL_PL_per_sec_per_irrad_N, bins=nbins, range=range_tuple, rwidth = 1, \
              align='mid', alpha = 1, edgecolor='k')#, normed = False,
              #histtype = 'barstacked', stacked=False)
...
#    ax.get_yaxis().set_ticks([])
    ax.set_ylim([0, 40])
    PL_median=round(np.median(list_of_TOTAL_PL_per_sec_per_irrad_N),1)
    PL_std=round(np.std(list_of_TOTAL_PL_per_sec_per_irrad_N),1)
    ax.axvline(np.median(PL_median), ymin = 0, ymax = 1, color='k', linestyle='--', linewidth=2,label="{0} ± {1}".format(PL_median, PL_std))
    plt.legend()
    ax.set_xlim(range_tuple)
    ax.set_xlabel(u'Total PL cross section (1e5 cts µm$^{2}$/mW seg EM)')
    ax.set_ylabel('Entries')
    aux_folder = manage_save_directory(path_from,'stats')
    figure_name = os.path.join(aux_folder,'hist_of_Total_PL_Cross_Section.png' )
    plt.savefig(figure_name)
    #plt.show()
    plt.close()
    
    mean_PL_file = os.path.join(aux_folder,'list_of_total_PL.dat')
    np.savetxt(mean_PL_file, list_of_TOTAL_PL_per_sec_per_irrad_N, delimiter=',', fmt='%.3e')
    

    
    ########## PLOT Histogram of all beta
...
#    ax.get_yaxis().set_ticks([])
   ...
   ########## PLOT HIST of Mean Beta Juli
    plt.figure()
    nbins = 10
    range_tuple = [30, 110]
    #list_of_mean_beta=list_of_mean_beta[list_of_mean_beta != 0]

    cleanedList = [x for x in list_of_mean_beta if str(x) != 'nan']
    cleanedList=[x for x in cleanedList if x != 0]
    plt.hist(cleanedList, bins=nbins, range=range_tuple, rwidth = 1, \
              align='mid', alpha = 1, edgecolor='k')#, normed = False,
              #histtype = 'barstacked', stacked=False)
    print(cleanedList)
    plt.legend(loc=1, handlelength=0)
    # [str(np.median(list_of_mean_beta)), '±' ,str(np.std(list_of_mean_beta))]
    ax = plt.gca()

    ax.axvline(np.median(cleanedList), ymin = 0, ymax = 1, color='k', linestyle='--', linewidth=2,label="{0} ± {1} - N={2}".format(round(np.median(cleanedList),1), round(np.std(cleanedList),1),len(cleanedList)))
#    ax.fill_between([borders_slope[0], borders_slope[1]], [0,0], [100,100],  facecolor='k', alpha=0.25)
#    ax.get_yaxis().set_ticks([])
#    ax.set_ylim([0, L])
    ax.set_xlim(range_tuple)
    ...
    ax.set_ylabel('Entries')
    aux_folder = manage_save_directory(path_from,'stats')
    figure_name = os.path.join(aux_folder,'hist_of_mean_beta_R2th_%s_%s_bines_juli.png' % \
                               (str(R2th), str(totalbins)))
    ...
    mean_beta_file = os.path.join(aux_folder,'list_of_mean.dat')
    np.savetxt(mean_beta_file, cleanedList, delimiter=',', fmt='%.3e')
    
   ########## PLOT HIST of Good Beta Juli
    
    extended_list_of_beta_good_array=[]

    for x in list_of_beta_good_array:
        if np.size(x)>0:
            if np.size(x)>1:
                for i in x:
                    extended_list_of_beta_good_array.append(i)
            else:
                extended_list_of_beta_good_array.append(x)        
    np.array(extended_list_of_beta_good_array).astype('float')
    extended_list_of_beta_good_array=np.array(extended_list_of_beta_good_array)
    #print(extended_list_of_beta_good_array)
    cleanedList = [x for x in extended_list_of_beta_good_array if str(x) != 'nan']
    plt.figure()
    nbins = 10
    range_tuple = [min(extended_list_of_beta_good_array),max(extended_list_of_beta_good_array)]
    plt.hist(cleanedList, bins=nbins, range=range_tuple, rwidth = 2, \
              align='mid', histtype='bar',alpha = 1, edgecolor='k',
               stacked=True)

    plt.legend(loc=1, handlelength=0)
    # [str(np.median(list_of_mean_beta)), '±' ,str(np.std(list_of_mean_beta))]
    ax = plt.gca()
    ax.axvline(np.median(cleanedList), ymin = 0, ymax = 1, color='k', linestyle='--', linewidth=2,label="{0} ± {1}".format(np.median(cleanedList), int(np.std(cleanedList))))
#    ax.fill_between([borders_slope[0], borders_slope[1]], [0,0], [100,100],  facecolor='k', alpha=0.25)
    ax.get_yaxis().set_ticks([])
    ax.set_ylim([0, len(extended_list_of_beta_good_array)/2])
    ax.set_xlim(range_tuple)
    ax.set_xlabel(u'Photothermal coefficient (K µm$^{2}$/mW)')
    ax.set_ylabel('Entries')
    aux_folder = manage_save_directory(path_from,'stats')
    figure_name = os.path.join(aux_folder,'hist_of_good_beta_R2th_%s_%s_bines_juli.png' % \
                                (str(R2th), str(totalbins)))
    plt.legend(loc='upper right')
    plt.savefig(figure_name)
  #  plt.show()
...
def statistics(path_from, R2th, totalbins, radius, sigma_abs, find_beta, find_Tzero):
    ...
    if find_beta and not find_Tzero:
        ...
        if number_of_NPs != 1:
            ...
            for i in range(number_of_NPs):
                ...
                out_hist = ax.hist(grep_betas_NPs, bins=nbins, range=range_tuple, rwidth = 1, \
                     align='mid', alpha = 1, color = 'C0', edgecolor='k', zorder=2, \
                     label='NP %d' % i)
        ...
        ####################################### hist BETA MEAN per SIZE Comento desde aca hasta 860
        #### ALL NPs IN ONE PLOT (histogram of its repetition)
        if number_of_NPs != 1:
            fig, axs = plt.subplots(number_of_NPs, 1, gridspec_kw = {'wspace':0, 'hspace':0})    
            for i in range(number_of_NPs):

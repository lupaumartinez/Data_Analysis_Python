...
from functions_for_photoluminiscence import manage_save_directory, \
    classification, calc_r2, closer, lorentz2, fit_lorentz2,instrumental_err, err_w0
...
def process_control_power_bfp(folder, poly):
...
def process_confocal_to_bins(folder, path_to, totalbins, image_size_px, 
                             ...
                             repetitions, mode, factor, control_power, calibration_power_BFP, meas_pow_bfp,start_notch, 
                             end_notch, start_power, end_power, start_spr, max_spr, 
                             lower_londa, upper_londa, exp_time_slow, w0_input, 
                             use_calibration_flag, last_bin_is_bkg_flag, EM, w0,plot_flag=False):
    ...
    if plot_flag:
        ...
        for i in range(image_size_px):
            for j in range(image_size_px):
                ...
                ax.plot(londa, matrix_spec_raw[i,j,:], color='C0', linestyle='-', label='As measured')
                ax.plot(londa_stokes, matrix_stokes_smooth[i,j,:], color='k', linestyle='-', label='Smoothed')
                ax.plot(londa_antistokes, matrix_antistokes_smooth[i,j,:], color='k', linestyle='-')
                ax.legend()
        ...
        for i in range(image_size_px):
            for j in range(image_size_px):
                ax.plot(londa, aux_matrix_spec_smooth2[i,j,:])
#                plt.legend()
...
#                ax.set_ylabel('Intensity (bkg corrected) (a.u.)' , fontsize=20)
        ...
        fig.savefig(figure_name, dpi=300, format='pdf')
        #plt.close()
    ...
    plt.axvline(londa[lower_londa_index], ymin = 0, ymax = 1, color='k', linestyle='--')
    plt.axvline(londa[upper_londa_index], ymin = 0, ymax = 1, color='k', linestyle='--')
    plt.axvline(londa[start_notch], ymin = 0, ymax = 1, color='k', linestyle='--')
    plt.axvline(londa[end_notch], ymin = 0, ymax = 1, color='k', linestyle='--')
    plt.axvline(londa[start_power], ymin = 0, ymax = 1, color='k', linestyle='--')
    plt.axvline(londa[end_power], ymin = 0, ymax = 1, color='k', linestyle='--')
    ...
    fig=plt.figure(num=1,clear=True)
    ax = fig.add_subplot()
    ax.plot(londa, bkg_raw, label='Raw')
    ax.plot(londa, bkg_smooth, '-k', label='Smooth')
    ax = plt.gca()
    ax.set_xlabel(r'Wavelength (nm)')
    ax.set_ylabel('Intensity (a.u.)')
    ax.set_ylim([y_min_spec_conf,y_max_spec_conf])
    ...
    fig=plt.figure(num=1,clear=True)
    ax = fig.add_subplot()
    ax.plot(londa, max_raw, label='Raw')
    ax.plot(londa, max_smooth, '-k', label='Smooth')
    ax = plt.gca()
    ax.set_xlabel(r'Wavelength (nm)')
    ax.set_ylabel('Intensity (a.u.)')
    ax.set_ylim([y_min_spec_conf,y_max_spec_conf])
    ...
    if plot_flag:
        ...
        for i in range(image_size_px):
            for j in range(image_size_px):
                ...
                ax = fig.add_subplot()
                pixel_name = 'i%02d_j%02d' % (i,j)
                ax.plot(londa, matrix_spec[i,j,:], label=pixel_name)
#                plt.legend()
                ax = plt.gca()
#                ax.set_ylim([y_min_spec_conf,y_max_spec_conf])
                ax.set_xlabel(r'Wavelength (nm)', fontsize=20)
...
#                plt.savefig(figure_name, dpi=300, format='pdf')
    ...
    for i in range(image_size_px):
        for j in range(image_size_px):
            pixel_name = 'i%02d_j%02d' % (i,j)
            ax.plot(londa, matrix_spec_normed[i,j,:], label=pixel_name)
    ax = plt.gca()
    ax.set_xlabel(r'Wavelength (nm)')
    ...
    TotalPL_sec_EM=np.sum(sum_stokes_spr)/(exp_time_slow*EM)
    ...
    fig=plt.figure(num=1,clear=True)
    ax = fig.add_subplot()
    ax.plot(londa_spr, sum_stokes_spr,'C0', label='Data %s' % NP)
    ax.plot(londa, full_lorentz_fitted,'k--', label='Lorentz fit')
    ax.legend()
    ...
    if control_power:
        mean_power_bfp, std_power_bfp = process_control_power_bfp(folder, calibration_power_BFP)
 
        meas_pow_sample = factor*mean_power_bfp
        err_meas_pow_sample = factor*std_power_bfp
    else:   
        meas_pow_sample = factor*meas_pow_bfp
        err_meas_pow_sample = instrumental_err*meas_pow_sample
    
    
    cal_irrad_calc = 2*meas_pow_sample/(np.pi*w0**2)
    err_cal_irrad_calc = np.sqrt( (2/(np.pi*w0**2)*err_meas_pow_sample)**2 +
    ...
    matrix_sum_Astokes_spec = np.sum(matrix_spec[:,:,:start_notch], axis=2)
    matrix_integrated_cts_per_sec = matrix_sum_Astokes_spec/exp_time_slow
  ...
  #  pixel_cts_per_sec = (matrix_integrated_cts_per_sec-min_integrated_cts_per_sec)/(max_integrated_cts_per_sec-min_integrated_cts_per_sec )
    ...
    fig=plt.figure(num=1,clear=True)
    ax = fig.add_subplot()
    img = ax.imshow(pixel_irrad_estimated, interpolation = 'none', cmap = 'magma')#'inferno')#'viridis')#'hot')
...
#        print(height_distro[i], 'between', I_1, I_2)
    ...
    fig=plt.figure(num=1,clear=True)
    ax = fig.add_subplot()
    ax.bar(centers, hist_bin, align='center')
    ax.errorbar(centers, hist_bin, fmt='', linestyle='', yerr=err_bin)
    ax.plot(centers_distro, height_distro, 'o-', color='C3')
    plt.xlabel('Bin')
...
#    print(max_max)
    ...
    for i in range(totalbins):
        ...
        ax.plot(londa, corrected_mean_specs[i]/max_max, 
                 color = color_iter,
...
#       plt.plot(londa, corrected_mean_specs[i], label='Bin %d' % i)
    ...
    plt.ylim([0, 1.02])
#    plt.ylim([0, y_max_spec_conf - y_min_spec_conf])
    fig.legend(loc='upper right', prop={'size':11})
    aux_folder = manage_save_directory(save_folder,'pl_in_bins')
...
#                 label='Group %d' % i)
    ...
    ax.tick_params(axis='both', which='major', labelsize=18)
    plt.xlim([505, 523])
    fig.legend(loc='lower right', prop={'size':12})
    ax.set_yscale('log')
    plt.ylim([0.005, 0.3])
    aux_folder = manage_save_directory(common_path, 'antistokes_in_bins')
...
#                 label='Group %d' % i)
    ...
    ax.tick_params(axis='both', which='major', labelsize=18)
#    plt.xlim([497, 523])
    fig.legend(loc='lower center', prop={'size':11})
    ax.set_yscale('log')
    plt.ylim([0.004, 1.25])
    aux_folder = manage_save_directory(common_path, 'antistokes_in_bins')
    ...
    PL_cts_per_sec_per_irrad=max_integrated_cts_per_sec/(cal_irrad_calc*EM)
    
    TotalPL_sec_EM_I=TotalPL_sec_EM/cal_irrad_calc
    
    to_save = np.array([max_integrated_cts_per_sec,PL_cts_per_sec_per_irrad,TotalPL_sec_EM_I])
    path_to_save = os.path.join(aux_folder,'PL_cts_per_sec_per_irrad_%s.dat' % NP)
    print(to_save)
    print(path_to_save)
    np.savetxt(path_to_save, to_save, fmt='%.3e')

...
def process_spectrum(folder, common_path, first_wave, starts_notch, ends_notch, last_wave, window, deg, repetitions, spectrum_time, fit_raman_water, fit_poly, plot_antistokes, fit_lorentz,  fit_bose_einstein, calibration_BS, sustrate_bkg_bool, trace_BS_bool):
    ...
    #    spectrum_original_smooth = smooth_Signal(spectrum_original, window, deg, repetitions)
      ...
      #  matrix_total[i, :] = spectrum
        ...
        spectrum_antistokes = smooth_Signal(spectrum_antistokes, window, deg, repetitions)
        
        spectrum_notch = spectrum[range_notch]
        
        matrix_total[i, range_stokes] = spectrum_stokes
        matrix_total[i, range_antistokes] = spectrum_antistokes
        matrix_total[i, range_notch] = spectrum_notch
        
        spectrum = matrix_total[i, :]
   ...
   # plt.xlim(-2, 242)
    ...
    if fit_lorentz:
       ...
       # time_width = frame_list[np.where(width_pl>0)[0]]
        ...
        if fit_poly: 
            plt.plot(time_poly, max_poly, '--*',  color = 'C3', label = '$\u03BB_{SPR}$ Poly fitting liveview' )
       ...
       # plt.plot(frame_list, new_C, '-*', label = 'Cte: fit raman water liveview')
        ...
        if fit_poly: 
            ...
            header_fitting = "Time (s), Max Poly SPR (nm)"
            data_fitting = np.array([time_poly, max_poly]).T
            name_data = os.path.join(save_folder, 'data_live_poly_fitting_%s.txt'%(NP))
            np.savetxt(name_data, data_fitting, header = header_fitting)
            
            if trace_BS_bool:
            
                print('Tiempo total de creecimiento:', 'por trace BS', time_BFP[-1], 'por spectrum live', time_poly[-1])
        
                header_fitting = "Total Time BS (s), Mean Power BS (mW), Total Time Spectrum (s), Max Polu SPR (nm)"
                data_fitting = np.array([time_BFP[-1], np.mean(power_BFP), time_poly[-1], max_poly[-1]]).T
                name_data = os.path.join(save_folder, 'data_growth_poly_final_%s.txt'%(NP))
                np.savetxt(name_data, data_fitting, header = header_fitting)
                
            else:
                
                print('Tiempo total de creecimiento:', 'por spectrum live', time_poly[-1])
        
                header_fitting = "Total Time BS (s), Mean Power BS (mW), Total Time Spectrum (s), Max Polu SPR (nm)"
                data_fitting = np.array([0, 0, time_poly[-1], max_poly[-1]]).T
                name_data = os.path.join(save_folder, 'data_growth_poly_final_%s.txt'%(NP))
                np.savetxt(name_data, data_fitting, header = header_fitting)
        ...
        if fit_raman_water: 
            ...
            if trace_BS_bool:
                ...
                print('Tiempo total de creecimiento:', 'por spectrum live', frame_list[-1])
        
                header_fitting = "Total Time BS (s), Mean Power BS (mW), Total Time Spectrum (s), Londa SPR live (nm)"
                data_fitting = np.array([0, 0, frame_list[-1], parameters_londa_spr[-1]]).T
                name_data = os.path.join(save_folder, 'data_growth_final_%s.txt'%(NP))
                np.savetxt(name_data, data_fitting, header = header_fitting)
...
#    daily_folder = '2021-08 (Growth PL circular)/2021-08-04 (pre growth, PL circular)/laser_polarizacion_circular_MAL/20210804-171638_Growth_10x1'
 #   daily_folder = '2021-08 (Growth PL circular)/2021-08-09 (growth, PL circular)/growth/Pruebas_concentracion_HAuCl4'
    
   # daily_folder = '2021-08 (Growth PL circular)/2021-08-09 (growth, PL circular)/growth/'
  #  list_daily_folder = [20210809-165810_Growth_10x2_col7y8_560nm', '20210809-172719_Growth_10x2_col9y10_570nm', '20210809-180739_Growth_10x2_col11y12_580nm',
  #                       '20210809-190759_Growth_10x1_col3_80s', '20210809-192328_Growth_10x1_col4_140s', '20210809-194745_Growth_10x1_col5_200s']
    ...
    for i in range(n, len(list_daily_folder[n:])+n):
       ...
       # list_of_folders = [f for f in list_of_folders if re.search('Liveview_Spectrum',f) and not re.search('Kinetics',f)]
        ...
        ends_notch = 545 #  #550 gold  #sustrate 700    #water  625 #antistokes 500
        last_wave = 645 # #570 gold  #sustrate 725   #water 670  #antistokes 521
        ...
        calibration_BS = np.array([6.07, -0.287]) #slope, intercept
        ...
        for f in list_of_folders:
            ...
            frame_list, wavelength, matrix_total, NP = process_spectrum(folder, path_to, first_wave, starts_notch, ends_notch, last_wave, window, deg, repetitions, spectrum_time,  fit_raman_water, fit_poly, plot_antistokes, bool_fit_lorentz, bool_fit_bose_einstein, calibration_BS, sustrate_bkg_bool, trace_BS_bool)
            plot_all = True
        ...
        if plot_all:
            ...
            step2.post2_process_spectrum(common_path, trace_BS_bool)

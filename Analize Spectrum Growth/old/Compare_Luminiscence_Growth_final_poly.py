...
def compare_final_spectrum(common_path_growth, common_path_PL, save_folder, name_col):
    ...
    for i in range(11):
      ...
      #  NP_PL = list_of_folders_PL[i].split('NP_')[-1]
        NP_PL = list_of_folders_PL[i-11].split('NP_')[-1]
        NP_PL = NP_PL.split('.')[0]
    ...
    #    wavelength = a[:, 0]
     ...
     #   fit_final_luminiscence = a[:, 2] #con sustrato
        ...
        desired_PL_final_growth = PL_final_growth[desired_range]
        ...
        name_file_PL = os.path.join(common_path_PL,  list_of_folders_PL[i-11])#list_of_folders_PL[i])  #list_of_folders_PL[i-11]
     ...
     #   wavelength_PL= b[:, 0]
       # luminiscence_NP = b[:, 1]
        ...
        desired_PL_steps_stokes = PL_steps_stokes[desired_range_PL]
     ...
     #   plt.plot(wavelength_PL, PL_steps, 'k', label = 'PL steps')
        plt.plot(wavelength_PL_stokes, PL_steps_stokes, label = 'PL steps stokes')
...
def compare_lspr(NP_growth, lspr_growth, NP_PL_list, lspr_PL, common_path_growth, common_path_PL, save_folder, name_col):
   ...
   # total_time = total_time[:11] #total_time[11:]
    ...
    NP = NP[11:]
    final_lspr_growth =  final_lspr_growth[11:]
    total_time = total_time[11:]
   ...
   # plot_time(total_time_growth, final_lspr_growth, lspr_PL, save_folder, name_col)
    ...
    header_txt = 'NP, lspr growth live (nm), Total time (s), lspr growth analysis poly (nm), lspr PL steps analysis poly (nm), lspr PL steps analysis two modes (nm)'
...
if __name__ == '__main__':
  ...
  #  daily_folder = '2021-03-10 (growth HauCl4 on AuNPz)'
   ...
   # sub_folder_PL = 'processed_data_luminiscence_sustrate_bkg_False_2/'
    ...
    save_folder = os.path.join(base_folder, daily_folder_PL, sub_folder_PL, 'POLY_fig_compare_PL_growth_final')
...
#%%
    ...
    for i in range(3):
      ...
      #  name_col = 'Col_%03d'%(number_col)
        name_col = 'Col_%03d_02'%(number_col) #tomo las primeras col  nada mas
    ...
    figure_name = os.path.join(save_folder, 'time_vs_final_lspr_PL_02.png') 
...
#%%
    ...
    folder = os.path.join(save_folder, 'info_PL_buenos_casos_POLY10')

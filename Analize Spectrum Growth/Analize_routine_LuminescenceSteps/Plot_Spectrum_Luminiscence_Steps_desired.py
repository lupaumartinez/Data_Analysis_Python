...
from Fit_raman_water import fit_signal_raman_test_two_modes, four_lorentz
...
def process_spectrum(folder, common_path, sustrate_bkg_bool, number_pixel, step_photo, window, deg, repetitions, desired_wavelength, fig_lim, fit_raman_water):
...
#    wavelength = np.loadtxt(calibration_wavelength)
    ...
    desired_specs = specs[number_pixel*(step_photo-1):number_pixel*step_photo]
    #smooth_specs = smooth_Signal(desired_specs, window, deg, repetitions)
    ...
    if fit_raman_water:
        ...
        wavelength_fitted, lorentz_fitted, best_parameters, r2 = fit_signal_raman_test_two_modes(desired_wavelength, desired_specs, ends_notch, last_wave, init_parameters, bounds)
        
       # lorentz_fitted = four_lorentz(wavelength_stokes, *best_parameters)
       # all_r2 = calc_r2(specs_stokes, lorentz_fitted)
        
        print('Fit Lorentz NP:', NP, *best_parameters)
        
    print('Ploteo:', NP)
    
    header_txt = 'Wavelength (nm), Spectrum'
    name_data = os.path.join(save_folder_2,'Luminescence_Steps_Spectrum_%s.txt'%(NP))
    data = np.array([desired_wavelength, desired_specs]).T
    ...
    plt.figure()      
    plt.title('%1s'%(NP))
    plt.plot(desired_wavelength, desired_specs, label = 'signal on NP+substrate')
    
    if fit_raman_water:
        plt.plot(wavelength_fitted, lorentz_fitted, '--g', label = 'fit lorentz two modes')
...
if __name__ == '__main__':
    ...
    #daily_folder = '2021-06 (Growth)/2021-06-19 (growth)/Luminescence_Steps/20210619-123551_Luminescence_Steps_12x2'  
  #  daily_folder = '2021-06 (Growth)/2021-06-18 (growth)/20210618-145900_Luminescence_Steps_12x10_pre_growth'
  ...
  #  daily_folder = '2021-08 (Growth PL circular)/2021-08-09 (growth, PL circular)/pre_growth/20210809-150932_Luminescence_Steps_10x12'
    ...
    subfolder = os.path.join(parent_folder, 'Spectrum_luminescence_Col_001_NP_001') #poner alguna para que lea wavelegnth
    
    list_of_files = os.listdir(subfolder)
    
    file1 = [f for f in list_of_files if re.search('Calibration',f)][0]
    
    calibration_wavelength = os.path.join(subfolder, file1)
    wavelength = np.loadtxt(calibration_wavelength) + 2 #esto es pq tenia mal puesto el gratting offset
    
    sustrate_bkg_bool = False
    
    number_pixel = 1002
    step_photo = 2 #2 siempre me interesa la región con center_wavelength en 590 nm
    
    window, deg, repetitions = 21, 0, 1
    plot_all_col = True  #plotea por cada columna todas sus NPs.
    fig_lim = -10, 4000
    
    fit_raman_water = True #fit lorentz to signal NP + substrate
    
    desired_wavelength = wavelength[number_pixel*(step_photo-1):number_pixel*step_photo]
    
    print(desired_wavelength[0], desired_wavelength[-1], len(desired_wavelength))
...
#Aca comienza la rutina
    ...
    path_to = os.path.join(parent_folder,'range_luminiscence_sustrate_bkg_%s'%sustrate_bkg_bool)
    ...
    for f in list_of_folders:
        ...
        process_spectrum(folder, path_to, sustrate_bkg_bool, number_pixel, step_photo, window, deg, repetitions, desired_wavelength, fig_lim, fit_raman_water)

...
def fit_lorentz(x, y, p, bounds):
    return curve_fit(lorentz, x, y, p0 = p, bounds = bounds)
...
def fit_four_lorentz(p, bounds, x, y):
    
    return curve_fit(four_lorentz, x, y, p0 = p, bounds = bounds)
...
def four_lorentz(x, *p):

    pi = np.pi

    I, gamma, x0, I2, gamma2, x02, I_2, I_3, C = p
    
    a = (1/pi) * I_2 * (15.5/2)**2 / ((x - 649)**2 + (15.2/2)**2) 
    b = (1/pi) * I_3 * (183/2)**2 / ((x - 702)**2 + (183/2)**2) 
    
    return  (1/pi) * I * (gamma/2)**2 / ((x - x0)**2 + (gamma/2)**2) + (1/pi) * I2 * (gamma2/2)**2 / ((x - x02)**2 + (gamma2/2)**2) + (1/pi) * I_2 * a + (1/pi) * I_3 * b + C
...
def fit_lorentz_signal_NP_two_modes(spectrum_S, wavelength_S, init_params, bounds):

    best_lorentz, err = fit_four_lorentz(init_params, bounds, wavelength_S, spectrum_S)
    
    wavelength_fitted = np.linspace(wavelength_S[0]-20,  wavelength_S[-1]+20, 1000)

    lorentz_fitted = four_lorentz(wavelength_fitted, *best_lorentz)
    
    lorentz_fit = four_lorentz(wavelength_S, *best_lorentz)
...
def fit_signal_raman(wavelength_NP, signal_NP, ends_notch, final_wave):
   ...
   # print('Fit Lorentz NP:', best_parameters, 'r2:', r2_coef_pearson)
    
    return wavelength_fitted, lorentz_fitted_NP, best_parameters

def fit_signal_raman_test(wavelength_NP, signal_NP, ends_notch, final_wave, init_parameters_NP, bounds):
   ...
   # print('Fit Lorentz NP:', best_parameters, 'r2:', r2_coef_pearson)
    ...
    return wavelength_fitted, lorentz_fitted_NP, best_parameters, r2_coef_pearson


def fit_signal_raman_test_two_modes(wavelength_NP, signal_NP, ends_notch, final_wave, init_parameters_NP, bounds):
   ...
   # specs_NP = smooth_Signal(specs_NP, window, deg, repetitions)
    ...
    lorentz_fitted_NP, wavelength_fitted, best_parameters, r2_coef_pearson = fit_lorentz_signal_NP_two_modes(spectrum_NP_S, wavelength_NP_S, init_parameters_NP, bounds)  

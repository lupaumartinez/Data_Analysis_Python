...
class Backend(QtCore.QObject):
    ...
    def update_image(self):
       ...
       # image = io.imread(self.f)
        ...
        first_calibration_and_image = np.loadtxt(direction + "/" + listfiles[0], dtype='float')
        
        calibration = np.array(first_calibration_and_image[:, 0])
        first_image = np.array(first_calibration_and_image[:, 1:])
        ...
        for n in range(0,len(listfiles)):
            stack[n,:,:]= np.array( np.loadtxt(direction + "/" + listfiles[n], dtype='float') [:, 1:])
            name_frames.append(listfiles[n].split(".asc")[0])
...
if __name__ == '__main__':
    ...
    base_folder = '/run/user/1000/gvfs/smb-share:server=fileserver,share=na/Luciana Martinez/Programa_Python/Analize Spectrum Growth/Analize_Scattering/'

    file = os.path.join(base_folder, 'lamparas_AndorSolis', 'lamp_IR_unpol.asc')
    #file = os.path.join(base_folder, 'lamparas_AndorSolis', 'LampIR_PX_image_1000.asc')
    #file = os.path.join(base_folder, 'lamparas_AndorSolis', 'LampIR_PY_image_1000.asc')
    
    lampara = np.loadtxt(file, dtype='float')

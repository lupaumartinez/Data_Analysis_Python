...
def procces_data_scattering(folder, col_folders, lamp):
    ...
    for number_col in col_folders:
        ...
        direction = os.path.join(folder, 'Col_%03d'%number_col)
    
        number_folder = direction.split('/')[-1]
   ...
   #     londa_fit = []
        ...
        window_smooth = 3
        window_smooth_2 = 21 #41 #antes 21, 41
        ...
        for i in range(len(data_spectrum)):
          ...
          #  spectrum_NP_steps = spectrum_NP_steps - spectrum_bkg_steps
            ...
            spectrum_NP = signal.savgol_filter(spectrum_NP, window_smooth_2, 0, mode = 'mirror')
        ...
        plt.savefig(save_folder + '/NP_Spectrums.png', dpi = 500)
        plt.show()
       ...
       # plt.plot(NP_list, londa_fit, '*', label = 'fit lorentz')
        ...
        plt.legend()
        plt.savefig(save_folder + '/Max_wavelength_NP_Spectrums.png', dpi = 500)
        plt.show()
        
        plt.hist(max_wavelength, bins=10, rwidth=0.9, color='C1')
        plt.xlabel('Max Wavelength (nm)')
        plt.ylabel('Frequency')
        plt.savefig(save_folder + '/hist_Max_wavelength_NP_Spectrums.png', dpi = 500)
        plt.show()
        
        plt.hist(max_wavelength_poly, bins=10, rwidth=0.9, color='C1')
        plt.xlabel('Max Wavelength Polynomial (nm)')
        plt.ylabel('Frequency')
        plt.savefig(save_folder + '/hist_Max_Poly_wavelength_NP_Spectrums.png', dpi = 500)
        plt.show()
...
if __name__ == '__main__':
    ...
    base_folder = 'C:/Users/lupau/OneDrive/Documentos/'
    daily_folder = 'Data_Paper_Growth/PL_dimero_08_Col_009_NP_087/post_growth/Scattering/Min_Pol'
       
    folder = os.path.join(base_folder, daily_folder)
    
    col_folders = [9] #[1,2,3,4,5]
    ...
    if mode == 'unpol':
       # file = os.path.join(folder_lamp, 'Resumen_lamparaIR_new', 'lamparaIR_grade_2.txt') 
        file = os.path.join(base_folder, 'Data_Paper_Growth/PL_dimero_08_Col_009_NP_087', 'lamparaIR_grade_2.txt') 
        
      #   file = os.path.join(base_folder, 'Data_Paper_Growth/Paper_growth_lenovo', '2021-11 (Growth PL circular)/2021-11-17 (Growth PL circular)/Scattering_unpol_slit320/lamp_IR_slit300', 'lamparaIR_grade_2.txt')
    if mode == 'px':
      #  file = os.path.join(folder_lamp, 'Resumen_lamparaIR', 'lamp_IR_PX_grade_2_PySpectrum.txt')
        file = os.path.join(base_folder, 'Data_Paper_Growth/Paper_growth_lenovo', '2021-08 (Growth PL circular)/2021-08-18 (post growth, PL circular)',
                          'Scattering_pol_min_laser', 'lampara_slit330_450-950nm_overlap_0.2',

...
def glue_steps(wave_PySpectrum, spectrum_py, number_pixel, grade, plot_all_step):
    ...
    spec_steps = np.zeros((number_pixel, L))
    wave_steps = np.zeros((number_pixel, L))
    
    spec_steps_glue = np.zeros((number_pixel, L))
    wave_steps_glue = np.zeros((number_pixel, L))
    ...
    for i in range(L):
        ...
        spec_steps_glue[:, i] = spec
        wave_steps_glue[:, i] = wave
        
        list_of_inf[i] = wave[0]
        list_of_sup[i] = wave[-1]
    ...
    for j in range(L-1):
       ...
       # sup = list_of_sup[i]
        ...
        desired_range_tail = range(number_pixel - m,  number_pixel)
    ...
    if plot_all_step:
        ...
        for i in range(L):
            plt.plot(wave_steps[:,i], spec_steps[:,i])
...
if __name__ == '__main__':
    ...
    lampara_analize = True#False#True# False
     ...
     #   subfolder = 'barrido_z_1um/Spectrum_luminescence_Col_001_NP_012'
      #  file_tiff = os.path.join(base_folder, daily_folder, subfolder, '20200108_162702_Picture_Andor_Spectrum_step_0005.tiff')
       # file_txt = os.path.join(base_folder, daily_folder, subfolder, '20200108_162702_Calibration_Shamrock_Spectrum_step_0005.txt') 
        ...
        save_folder2 = os.path.join(base_folder, daily_folder, 'Slit150um/IR_unpol/0.5s/')
        file_tiff2 = os.path.join(save_folder2, '20200108-143110_Spectrum_Measurment_step_08_IR_unpol', '20200108_143110_Picture_Andor_Spectrum_step_0008.tiff')
        file_txt2 = os.path.join(save_folder2, '20200108-143110_Spectrum_Measurment_step_08_IR_unpol', '20200108_143110_Calibration_Shamrock_Spectrum_step_0008.txt')   
        file_tiff_dark2 = os.path.join(save_folder2, '20200108-143411_Spectrum_Measurment_step_08_dark', '20200108_143411_Picture_Andor_Spectrum_step_0008.tiff')
   ...
   # image_PySpectrum2 = np.transpose(io.imread(file_tiff2))
   # wave_PySpectrum2  = np.array(np.loadtxt(file_txt2)) 
   # image_PySpectrum_dark2 = np.transpose(io.imread(file_tiff_dark2)) 
    #image_PySpectrum2 = image_PySpectrum2 - image_PySpectrum_dark2
        
    grade = 3
    number_pixel = 1002
    
    #First ROI, then Glue
    #spectrum_py2 = select_ROI(image_PySpectrum2, center_row, spot_size)
    #wave_final2, spectrum_final2 = glue_steps(wave_PySpectrum2, spectrum_py2, number_pixel, grade, plot_all_step = True)
    #norm_PySpectrum2 = spectrum_final2/max(spectrum_final2)
  ...
  #  plt.figure(101)
  #  plt.plot(wave_final, norm_PySpectrum, 'k*-', label = '1s')
  #  plt.plot(wave_final2, norm_PySpectrum2, 'g*-', label = '0.5s')
  #  plt.legend()
   # plt.show()

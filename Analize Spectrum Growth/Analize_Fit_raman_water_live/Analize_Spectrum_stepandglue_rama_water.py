...
class Frontend(QtGui.QFrame):
    ...
    lineparametersSignal = pyqtSignal(int, int, int, bool, int, int, bool)
    ...
    fit_raman_Signal = pyqtSignal(bool, int, int)
    ...
    def setUpGUI(self): 
        ...
        self.fit_raman_option = QtGui.QCheckBox('Fit raman of water')
        self.fit_raman_option.clicked.connect(self.fit_raman_check)
...
# 'cyclic', 'greyclip', 'grey' # Solo son estos
        ...
        self.curve_fit_spectrum = self.lineplot_spectrum_Widget.linePlot.plot(open='y')
    ...
    def create_line_Horizontal(self):
        ...
        if self.lineHorizontal_button.isChecked():
            ...
            self.lineparametersSignal.emit(self.frame, self.center_row, self.spot_size, self.normalized_bool, self.center_bkg, self.window_smooth, self.fit_raman_bool)
    ...
    def get_peak_local_maximum_position(self):
        ...
        if values_empty_NP[0] != 0:
            ...
            name_NP_list = list(np.arange(0,total_NP ,1))
    ...
    @pyqtSlot(np.ndarray, np.ndarray)
    def fit_spectrum_line_show(self, array_wavelength, array_profile):
    ...
    def make_connection(self, backend):
        ...
        backend.line_fit_spectrumSignal.connect(self.fit_spectrum_line_show)
...
class Backend(QtCore.QObject):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.lower_wavelength = 545
        self.upper_wavelength = 1000
    ...
    def update_image(self):
       ...
       # image = io.imread(self.f)
        ...
        for n in range(0,len(listfiles)):
            ...
            name_frames.append(listfiles[n].split(".tiff")[0])
            
        #self.image = stack[:,1:,:]  #solo porq guarde mal StepandGlue
        self.image = stack
        self.name_frames = name_frames
        ...
        self.line_spectrum_parameters(self.frame, self.center_row, self.spot_size, self.normalized_lamp_bool, self.center_bkg, self.window_smooth, self.fit_raman_bool)
    ...
    @pyqtSlot(int, int, int, bool, int, int, bool)    
    def line_spectrum_parameters(self, frame, center_row, spot_size, normalized_lamp_bool, center_bkg, window_smooth, fit_raman_bool):
        ...
        self.fit_raman_bool = fit_raman_bool
    ...
    def line_spectrum(self, calibration, image):
      ...
      #  spectrum = np.round(np.sum(image[:,roi_rows], axis=1),2)
    ...
    def spectrum_NPs(self, info_NPs, spot_size, center_bkg, window_smooth):
        ...
        name_frame = self.name_frames[frame]

        folder = self.create_folder(self.file_path, name_frame, normalized = False)
        folder_normalized = self.create_folder(self.file_path, name_frame, normalized = True)
        ...
        for i in range(len(center_NPs)):
            ...
            print('Save col', name_frame)

    def save_spectrum(self, axe_x, axe_y, NP, folder):

        file_path = folder
        file_name = "NP_%03d.txt"%(NP)
    ...
    def save_spectrum_bkg(self, axe_x, axe_y, row, folder):
        ...
        file_path = folder
        file_name = "background_row_%04d.txt"%(row)

        fullname = os.path.join(file_path, file_name)
        np.savetxt(fullname, np.transpose([axe_x, axe_y]))
        
    def create_folder(self, file, name_frame, normalized):

        file_path = file
        
        if not normalized:
        
            name_folder = "%s"%(name_frame) #Col_%2d"%(number)
            folder = os.path.join(file_path, name_folder)
            
        else:
            
            name_folder = "normalized_%s"%(name_frame) # "normalized_Col_%2d"%(number)
            folder = os.path.join(file_path, name_folder)
...
if __name__ == '__main__':
  ...
  #  lampara = np.loadtxt('LampIR_PX_image_1000.asc', dtype='float')
  #  lampara = np.loadtxt('LampIR_PY_image_1000.asc', dtype='float')

...
def image_confocal_stokes(parent_folder, f, plot):
    ...
    files = [f for f in list_of_folders if re.search('Slow_Confocal',f)]
...
def binning(path_folder, NP, matrix, px, py, totalbins, plot):
    
    binned_power = np.zeros((py, px))
...
def calc_irradiance(factor, w0, err_w0, matrix, power_mean, power_std, calc_power):
    
    pixel_cts_per_sec = matrix/np.max(matrix)
...
def bins_irradiance(path_folder, NP, matrix_irradiance, matrix_std_irradiance, totalbins, binned_power):
   ...
   # plt.ylabel(u'Irradiance (mW/nm$^{2}$)')
    plt.ylabel(u'Norm PL Stokes')
    plt.xlabel('Bin')
    figure_name = os.path.join(path_folder,'PL_Stokes_in_bins_%s.png' % NP)
...
def binning_distance(path_folder, NP, totalbins, binned_power, size_image_x, size_image_y, px, py):
    
    pixel_size = size_image_x/px
    axe = np.arange(-size_image_x/2+pixel_size/2, size_image_x/2+pixel_size/2, pixel_size)
...
if __name__ == '__main__':
    ...
    daily_folder = r'2022-06-03 AuNP 67 impresas\20220603-124258_Luminescence_10x2_3.0umx3.0um'

    parent_folder = os.path.join(base_folder, daily_folder)
    
    save_folder = os.path.join(parent_folder, 'psf_bins10')
    ...
    x0 = 0
    axe = np.arange(0, size_image_x/2, 2)
    ...
    for f in list_of_folders_slow[:]:
        ...
        if number_NP in exceptions_NP:
           print('salteo NP', number_NP)
           continue
        ...
        matrix_irradiance, matrix_std_irradiance = calc_irradiance(factor, w0, err_w0, matrix_stokes, power_mean, power_std, calc_power)
       
        binned_power = binning(save_folder, NP, matrix_stokes, px, py, totalbins, plot = False)
        ...
        mean_distance, std_distance = binning_distance(save_folder, NP, totalbins, binned_power, size_image_x, size_image_y, px, py)
   ...
   # plt.plot(axe, I, 'k--')
    ...
    plt.ylabel(u'Norm PL Stokes')
   # plt.ylabel(u'Irradiance (mW/nm$^{2}$)')
    figure_name = os.path.join(save_folder,'pl_stokes_vs_distance_of_bins.png')
    plt.savefig(figure_name, dpi = 300)
    plt.show()
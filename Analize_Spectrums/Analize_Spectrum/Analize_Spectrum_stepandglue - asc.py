    ...
    def line_spectrum_normalized_lamp(self, calibration, image, lamp, center_row, window_smooth):
      ...
      #  smooth_spectrum_NP =  signal.savgol_filter(smooth_spectrum_NP, window_smooth, 1)
       ...
       # smooth_spectrum_bkg =  signal.savgol_filter(smooth_spectrum_bkg, window_smooth, 1)
        ...
        spectrum_lamp =  signal.savgol_filter(spectrum_lamp, window_smooth, 1)
      #  spectrum_lamp =  signal.savgol_filter(spectrum_lamp, window_smooth, 1)
      #  spectrum_lamp =  signal.savgol_filter(spectrum_lamp, window_smooth, 1)
        
        spectrum_lamp = self.interpolated_lamp(wavelength, [lamp[0], spectrum_lamp])
        ...
        desired_range = np.where((wavelength>=450) & (wavelength<=1000))
        desired_wavelength = wavelength[desired_range]
        desired_spectrum = spectrum_NP_normalized[desired_range]
        
        return desired_wavelength, desired_spectrum
...
if __name__ == '__main__':
    ...
    smooth_espectro_lampara  =  signal.savgol_filter(espectro_lampara , 11, 1)

...
#Iani experimento NP Au en meso Tio2, lampara halogena vieja
#data = np.loadtxt('Col1PXt1.asc')
#lampara = np.loadtxt('lamp_unpol_40_overlap_400_to_1000_13MHz_37x_1sec_95_offset_2_acum.asc')

#Chechu experimento NP Silicio, lampara halogena nueva
#data = np.loadtxt('Col1PXt1_Chechu.asc')
#data_calibration = np.array(data[:, 0])
#data_image = np.array(data[:, 1:])

#lampara = np.loadtxt('calibracion_lampara_unpol_2_IR_20190805.asc', dtype='float')
...
smooth_espectro_lampara  =  signal.savgol_filter(espectro_lampara, 3, 1)
smooth_espectro_lampara  =  signal.savgol_filter(smooth_espectro_lampara , 3, 1)
smooth_espectro_lampara  =  signal.savgol_filter(smooth_espectro_lampara , 3, 1)
...
class Backend(QtCore.QObject):
    ...
    def line_spectrum_normalized_lamp(self, calibration, image, lamp, center_row, window_smooth):
       ...
       # spectrum_bkg = spectrum_bkg[desired_range]
        ...
        smooth_spectrum_NP =  signal.savgol_filter(spectrum_NP, window_smooth, 1)
        smooth_spectrum_NP =  signal.savgol_filter(smooth_spectrum_NP , window_smooth, 1)
        smooth_spectrum_NP =  signal.savgol_filter(smooth_spectrum_NP , window_smooth, 1)
        
        smooth_spectrum_bkg =  signal.savgol_filter(spectrum_bkg, window_smooth, 1)
        smooth_spectrum_bkg =  signal.savgol_filter(smooth_spectrum_bkg, window_smooth, 1)
        smooth_spectrum_bkg =  signal.savgol_filter(smooth_spectrum_bkg, window_smooth, 1)
             
        smooth_spectrum_lamp =  signal.savgol_filter(lamp[1], window_smooth, 1)
        smooth_spectrum_lamp =  signal.savgol_filter(smooth_spectrum_lamp , window_smooth, 1)
        smooth_spectrum_lamp =  signal.savgol_filter(smooth_spectrum_lamp , window_smooth, 1)
    ...
    def spectrum_NPs(self, info_NPs, spot_size, center_bkg, window_smooth):
        ...
        total_frame = int(self.image.shape[0])

        folder = self.create_folder(self.file_path, total_frame, normalized = False)
        folder_normalized = self.create_folder(self.file_path, total_frame, normalized = True)
        ...
        for j in range(total_frame):
            ...
            for i in range(len(center_NPs)):
                ...
                NP = int(name_NPs[i])
                wavelength, spectrum_NP = self.line_spectrum(self.calibration, imagen)
                self.save_spectrum(wavelength, spectrum_NP, NP, j, folder)
                
                wavelength, spectrum_bkg = self.line_spectrum_bkg(self.calibration, imagen)
                self.save_spectrum_bkg(wavelength, spectrum_bkg, center_bkg, j, folder)
                ...
                self.save_spectrum(wavelength, spectrum_NP_normalized, NP, j, folder_normalized)


    def save_spectrum(self, axe_x, axe_y, NP, frame, folder):

        file_path = folder
        file_name = "NP_%2d_frame_%2d.txt"%(NP, frame)

        fullname = os.path.join(file_path, file_name)
        np.savetxt(fullname, np.transpose([axe_x, axe_y]))
        
    def save_spectrum_bkg(self, axe_x, axe_y, row, frame, folder):

        file_path = folder
        file_name = "background_row_%2d_frame_%2d.txt"%(row, frame)

...
def calculate_quotient(folder, path_to, totalbins, lower_londa, 
    ...
    thermometry_using_scattering = False
    ...
    if thermometry_using_scattering:
        ...
        plt.close()  
        plt.close('all')
        x_long = np.arange(2.12, 2.22, 0.01)    
...
#        ax.axvline(energy[b], ymin=0, ymax=1, linestyle='--', color='k')
    ...
    for ref in list_of_bins:
        ...
        ax = fig.add_subplot()
        for i in list_of_bins:    
            ax.plot(xdata_londa, ydata_all[i,ref,:], '-', label='Bin %d/%d - A %.2f' % (i, ref, bin_A_array[i, ref]))
        ax.grid(True)
        ax.legend(loc='best')
    ...
    for ref in list_of_bins:
        ...
        for i in list_of_bins_to_fit:
            ...
            if find_beta and not find_Tzero:
                ...
                ax.plot(x_long, yfit_to_plot, '--k', alpha = 0.8)
                color_iter = colormap[i]
...
#        fig.close()    
    ...
    fig=plt.figure(num=1,clear=True)
    ax = fig.add_subplot()
    ax.imshow(beta_matrix, interpolation='none', cmap='plasma')
    ...
    fig=plt.figure(num=1,clear=True)
    ax = fig.add_subplot()
    ax.imshow(Tzero_matrix, interpolation='none', cmap='plasma')
    ...
    fig=plt.figure(num=1,clear=True)
    ax = fig.add_subplot()
    ax.imshow(T_matrix, interpolation='none', cmap='plasma')
    ...
    ax.imshow(R2_matrix, interpolation='none', cmap='viridis')
    for i in range(totalbins):
        for j in range(totalbins):
    ...
    fig=plt.figure(num=1,clear=True)
    ax = fig.add_subplot()
    ax.imshow(p_value_matrix, interpolation='none', cmap='viridis')
    for i in range(totalbins):
        for j in range(totalbins):

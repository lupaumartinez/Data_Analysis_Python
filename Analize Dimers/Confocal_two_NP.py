...
def image_two_NP(N, rango, Io, sigma_x, sigma_y, xo, yo, C, Io_2, xo_2, yo_2):
	...
	p = Io, sigma_x, sigma_y, xo, yo, C
	p2 = Io_2, sigma_x, sigma_y, xo_2, yo_2, 0

	for i in range(N):
		for j in range(N):
			image[i, j] = gaussian_2D(x[i], y[j], *p) +  gaussian_2D(x[i], y[j], *p2) + 0.01*Io*np.random.normal(0, 1)

	return image

def filter_image(image, n, type_image):

	Z = image

	if type_image == 'maximum':

		Zn = (image-np.min(image))/(np.max(image)-np.min(image))
...
def center_mass(image, n_filter, range_x, range_y):
	...
	com_nm = (np.array(com[0]) - Nx/2)*px_nm, (np.array(com[1]) - Ny/2)*py_nm
...
def two_center_gauss(image, initial_position_1, initial_position_2, range_x, range_y):
	...
	initial_sigma = [125/px_nm, 135/py_nm]
	bounds = ([0, -Nx/2,- Ny/2, 0, 0, 0, 0, -Nx/2,- Ny/2], [1, Nx/2, Ny/2, 4*initial_sigma[0], 4*initial_sigma[0], 1, 1, Nx/2, Ny/2])

	initial_guess_G = [1, initial_position_1[0] - Nx/2,  initial_position_1[1] - Ny/2, initial_sigma[0], initial_sigma[1], 0, 1, initial_position_2[0] - Nx/2,  initial_position_2[1] - Ny/2]
	...
	cog_1 = poptG[2] + Nx/2 - 1/2, poptG[1] + Ny/2 - 1/2
	cog_2 = poptG[8] + Nx/2 - 1/2, poptG[7] + Ny/2 - 1/2
	#cog = poptG[2], poptG[1]

	cog_1_nm = (np.array(cog_1[0]) - Nx/2 + 1/2)*px_nm, (np.array(cog_1[1]) - Ny/2 + 1/2)*py_nm
	cog_1_nm = np.around(cog_1_nm, 2)

	cog_2_nm = (np.array(cog_2[0]) - Nx/2 + 1/2)*px_nm, (np.array(cog_2[1]) - Ny/2 + 1/2)*py_nm
...
if __name__ == '__main__':
    ...
    if plot_one_NP:
        ...
        image = image_NP(N= N, rango=rango, Io=A, sigma_x =sigma_x, sigma_y = sigma_y, xo = xo, yo = yo, C = C)
        ...
        t0 = time.time()    
        com = center_mass(image, n_filter, range_x = rango , range_y = rango)
        t1 = time.time()
        t_m = (t1-t0)*1e3
        ...
        t0 = time.time()
        cog = center_gauss(image, initial_pos, range_x = rango, range_y = rango)
        t1 = time.time()
        t_G = (t1-t0)*1e3 
    ...
    if plot_two_NP:
        ...
        if image_real:
            ...
            image = io.imread(name_file)
                ...
                image_2 = image_NP(N= N, rango=rango, Io=A, sigma_x =sigma_x, sigma_y = sigma_y, xo = 500, yo =  0, C = C)
                #image_2 = image_two_NP(N= N, rango=rango, Io=A, sigma_x =sigma_x, sigma_y = sigma_y, xo = xo, yo = yo, C = C, Io_2 = Io2, xo_2 = xo2, yo_2=yo2)
                image_1 = image_NP(N= N, rango=rango, Io=A, sigma_x =sigma_x, sigma_y = sigma_y, xo = 0, yo = 0, C = C)
                image = image_1 + image_2
    
        t0 = time.time()
        image_2 = filter_image(image, 0.15, 'maximum')
        t1 = time.time()
        t_f = (t1-t0)*1e3 
        ...
        axe, profile_x, profile_y = curve_gauss(image_2, rango = rango)
        ...
        t0 = time.time()
        cog_1, cog_2 = two_center_gauss(image, initial_pos_1, initial_pos_2, range_x = rango, range_y = rango)
        t1 = time.time()
        t_G = (t1-t0)*1e3 
